# Hey, I'm David

Senior Technologist, Advisor. Founder at [All Tuner Labs](https://alltuner.com).

## Some of my projects

- 🏭 [**Factory Floor**](https://factory-floor.com) ([GitHub](https://github.com/alltuner/factoryfloor)) — AI-powered development workspace for macOS with integrated git worktrees and Claude Code
- 🌙 [**a-clapar.cat**](https://a-clapar.cat) ([GitHub](https://github.com/alltuner/a-clapar)) — Nightly AI-narrated bedtime stories podcast in Catalan
- 🎙️ [**Castuner**](https://castuner.com) — A platform to aid creators in reaching their audiences
- ⏱️ [**Shiftuner**](https://shiftuner.com) ([GitHub](https://github.com/alltuner/shiftuner)) — Compliance-focused time tracking for Spanish companies
- 🚆 [**retards.cat**](https://retards.cat) — Real-time schedules, delays and route maps for Barcelona's Rodalies
- ⚡ [**Vibetuner**](https://github.com/alltuner/vibetuner) — Production-ready FastAPI scaffolding with HTMX and OAuth
- 📦 [**uv-version-bumper**](https://github.com/alltuner/uv-version-bumper) — GitHub Action to bump uv-managed Python project versions
- 🐚 [**mise-completions-sync**](https://github.com/alltuner/mise-completions-sync) — Sync shell completions for tools managed by mise
- 🔐 [**Vaultuner**](https://github.com/alltuner/vaultuner) — Secrets management toolkit for development workflows
- 🍪 [**Cookietuner**](https://github.com/alltuner/cookietuner) — Extract and display cookies from macOS browsers
- 📰 [**substack-extractor**](https://github.com/davidpoblador/substack-extractor) — Crawl and extract text content of Substack posts

## Writing

- [**Tech Diet Club**](https://davidpoblador.com/blog/) — Practical tech insights, system design decisions, and architecture patterns
- [**Fils per randa**](https://poblador.cat/blog/) — Reflexions sobre tecnologia que realment compten

## Support

If you find any of these projects useful, consider [sponsoring our work](https://alltuner.com/sponsor/) via [GitHub Sponsors](https://github.com/sponsors/alltuner) or [Buy Me a Coffee](https://buymeacoffee.com/alltuner).

## Links

- [davidpoblador.com](https://davidpoblador.com)
- [poblador.cat](https://poblador.cat)
- [LinkedIn](https://www.linkedin.com/in/davidpoblador)
- [Bluesky](https://bsky.app/profile/davidpoblador.com)
- [X](https://x.com/davidpoblador)
